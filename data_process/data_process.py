...
def stages_data_process(input_path,output_path,plc_ind):
    input = np.load(input_path, allow_pickle=True)
    new_output = single_plc_set(output_path,plc_ind)
    return input,new_output

def actuators_data_process(input_path,output_path,plc_ind,actuator_ind):
    input = np.load(input_path, allow_pickle=True)
    new_output = single_actuator_set(output_path,plc_ind,actuator_ind)
    return input,new_output
